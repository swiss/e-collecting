...
block
  block
    ...
    a1["Variante 1:
    Solution basée sur un
    navigateur avec authentification
    via un smartphone ou un
    jeton matériel (aucune
    application e-collecting dédiée
    n’est requise)"]
    a2["Variante 2:
    Utilisation possible à la
    fois via un navigateur et
    via une application e-collecting
    installée sur un smartphone"]
    a3["Variante 3:
    Utilisation uniquement via
    une application e-collecting
    installée sur un smartphone "]
...
block
  block
    ...
    t2["Ausprägungen"]:3
    d["Parameter 10
    Bereitstellungsform der
    E-Collecting Anwendung"]
    a1["Ausprägung 1:
    Browserbasiert mit Smartphone-
    oder Hardware-Token gestützter
    Authentifizierung (keine eigene
    E-Collecting-App erforderlich)"]
    a2["Ausprägung 2:
    Nutzung sowohl browserbasiert
    als auch über eine installierte
    E-Collecting-App auf
    Smartphone möglich"]
    a3["Ausprägung 3:
    Nutzung nur über installierte
    E-Collecting-App auf Smartphone"]
    style t1 fill:#a8d5ff,stroke-width:0px
    style t2 fill:#a8d5ff,stroke-width:0px
    style d fill:#a8d5ff,stroke-width:0px,text-valign:top
    style a1 fill:#cdedff,stroke-width:0px
